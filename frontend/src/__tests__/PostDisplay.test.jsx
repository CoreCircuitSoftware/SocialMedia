...
describe("PostDisplay", () => {
	...
	const mockUserData2 = {
		...
		displayName: "abc",
		profilePicture:
			"https://i.redd.it/fat-tux-i-created-this-simple-tux-logo-for-fun-v0-w9zk1c24wgz91.png?auto=webp&s=78ae0dec3e65420a44af1ad983f9b708b3efb73f",
		bio: "",
		backgroundColor: "",
		backgroundImage: "",
	};
	...
	it("renders the default details correctly", async () => {
		render(<PostDisplay post={mockSlug} curUser={mockCurUser} />);
	...
	it("Share button copies link of post to clipboard", async () => {
		...
		await waitFor(() => {
			...
			expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
				`http://circuitsocial.tech/post/view/${mockSlug.post.postID}`,
			);
		});
	});
	...
	it("Edit button navigates to edit page", async () => {
		...
		await waitFor(() => {
			...
			expect(mockNavigate).toHaveBeenCalledWith(
				`/post/edit/${mockSlug.post.postID}`,
			);
		});
	});
	...
	it("Comments button brings user to comments page", async () => {
		...
		await waitFor(() => {
			...
			expect(mockNavigate).toHaveBeenCalledWith(
				`/post/view/${mockSlug.post.postID}`,
			);
		});
	});

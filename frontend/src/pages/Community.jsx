...
export default function CommunityTest() {
	...
	const handlePostCreate = () =>
		navigate(`/post/create/${community.communityID}`);

	const handleShare = async () => {
		try {
			await navigator.clipboard.writeText(
				`http://circuitsocial.tech/profile/${profile.username}`,
			);
			// alert("Copied");
		} catch (err) {
			...
			<Box sx={{ flexGrow: 1, marginLeft: "300px", mt: 8 }}>
				<div className="content">
					<div className="profile-top">
						<img
							className="back-img"
							src="https://django-s3-4800.s3.us-east-2.amazonaws.com/post_images/background_pG2GYnu.jpg"
							alt="background"
							data-cy="banner"
						/>
						<div className="profile-card">
							<div className="card-upper">
								...
								/>
								<div className="names">
									<p className="display-name" data-cy="display-name">
										{community.name}{" "}
									</p>
								</div>
											...
											>
												...
												{!isMember ? (
													<Button
														...
														data-cy="edit"
													>
														{isMember ? "Following" : "Follow Community"}{" "}
														{/* Button text changes based on isMember */}
													</Button>
												) : (
													<Button
														variant="contained"
							...
							{true ? (
								...
								<h3 data-cy="user-no-posts">
									{communityname} is empty, be the first to post!?
								</h3>
							)}
						</div>
					</div>

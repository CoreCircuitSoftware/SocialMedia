...
export default function PostPage() {
	...
	const getVoteTotals = async () => {
		...
		api.get(`/api/posts/vote/gettotal/${post.postID}/`).then((res) => {
	...
	const changeVoteCountLocally = (voteType, e) => {
	...
	const handleVote = (voteType) => {
		if (postVote == -1) {
			...
			api.delete(`/api/posts/vote/delete/${post.postID}/`);
			setPostVote(-1);
		} else {
			changeVoteCountLocally(voteType, 2);
			api.patch(`/api/posts/vote/update/${post.postID}/`, { vote: voteType });
			setPostVote(voteType);
		}
	};
	...
	const handleShare = async () => {
		...
		try {
			await navigator.clipboard.writeText(
				`http://circuitsocial.tech/post/view/${post.postID}`,
			);
		} catch (err) {
			...
			<AppBar position="fixed">
				<Toolbar sx={{ display: "flex", alignItems: "center", width: "102%" }}>
					...
					>
						<Typography
							variant="h6"
							sx={{ textAlign: "center", marginRight: 1 }}
						>
							CircuitSocial
						</Typography>
						<SearchBar />
			...
			<div className="content" style={{ marginTop: "40px" }}>
				<div className="main-post">
					<div className="post-page-options-buttons">
						<button
							className="pfp-post-main-btn"
							onClick={handleProfileClick}
							data-cy="profile-picture"
						>
							<img className="pfp-post-main" src={thisUser.profilePicture} />
						</button>
					...
					<p className="post-description" data-cy="post-description">
						{post.description}
					</p>
					...
					<h5 className="post-date" data-cy="post-date">
						{formattedDate}
					</h5>
					{post.hasEdit && (
						<h6 className="edit-date">Edited: {formattedEditDate}</h6>
					)}
				...
				<div className="comments">
					{comments.length > 0 ? (
						comments.map((comment) => (
							<CommentDisplay comment={comment} key={comment.commentID} />
						))
					) : (

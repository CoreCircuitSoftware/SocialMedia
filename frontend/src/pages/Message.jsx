...
export default function MessagePage() {
	...
	useEffect(() => {
		if (!convoID || convoID.length === 0) {
			console.warn(
				"WebSocket connection aborted: convoID is undefined or empty.",
			);
			return;
		}
	...
	const addParticipant = (participant, newConvoID) => {
		try {
			api.post("/api/profile/message/setconvoparticipant/", {
				user: participant,
				convo: newConvoID,
			});
		} catch (error) {
	...
	useEffect(() => {
		//check if both user's have been found yet
	...
	return (
		<main>
			...
			<Menu />
			<MessageListPage
				onConvoSelect={handleConvoSelection}
				getOtherUser={displayOtherUser}
			/>
			{/* Main Content */}
			<div
				style={{
										...
										style={{
											...
											justifyContent:
												message.sender === myProfile.id
													? "flex-end"
													: "flex-start", // Align messages correctly

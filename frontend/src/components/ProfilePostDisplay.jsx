...
export default function PostDisplay(slug) {
	...
	const formattedEditDate = new Date(thisPost.editDate).toLocaleString(
		"en-US",
		{
			year: "numeric",
			month: "long",
	...
	const getCommentsTotal = async () => {
		api.get(`/api/posts/comments/gettotal/${thisPost.postID}/`).then((res) => {
	...
	const getVoteTotals = async () => {
		...
		api.get(`/api/posts/vote/gettotal/${thisPost.postID}/`).then((res) => {
	...
	const handlePostShare = async () => {
		...
		try {
			await navigator.clipboard.writeText(
				`http://circuitsocial.tech/post/view/${thisPost.postID}`,
			);
		} catch (err) {
					...
					<header>
						...
						<div className="name-text">
							{thisPost.community != null ? (
								<h1 onClick={goToCommunity}>
									{community.name} @{thisUser.username}
								</h1>
							) : (
								<h1>
									{thisUser.displayName} @{thisUser.username}
								</h1>
							)}
						</div>
					</header>
            ...
            <h5 className="post-date">{formattedDate}</h5> */}
						...
						{isMyPost ? (
							<div>
								<ButtonGroup variant="contained">
									<Button
										onClick={() => navigate(`/post/edit/${thisPost.postID}`)}
									>
										edit
									</Button>
					...
					<div className="post-options">
						<ButtonGroup variant="contained">
							...
							<Button startIcon={<ChatBubble />} onClick={handlePostClick}>

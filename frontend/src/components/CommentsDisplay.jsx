...
export default function CommentDisplay(slug) {
	...
	const handleVote = (voteType) => {
		if (commentVote == -1) {
			...
			api
				.patch(`/api/comment/vote/update/${comment.commentID}/`, {
					vote: voteType,
				})
				.catch((err) => console.log(err));
			setCommentVote(voteType);
	...
	useEffect(() => {
		(getUser(), getVoteTotals(), getCommentRepliesTotal());
	}, []);
	useEffect(() => {
		(getVote(), getMyProfile());
	}, [thisUser]);

	const navigate = useNavigate();
	const handleProfileClick = () => navigate(`/profile/${thisUser.username}`);
	const handleCommentClick = () =>
		navigate(`/comment/view/${comment.commentID}`);
	const handleCommentsShare = async () => {
		event.preventDefault();
		try {
			await navigator.clipboard.writeText(
				`http://circuitsocial.tech/comment/view/${comment.commentID}`,
			);
		} catch (err) {
		...
		<div className="comment_main">
			{thisUser ? (
				<div className="comment_container">
					...
					{comment.hasEdit && (
						<h6 className="comment-date">Edited: {formattedEditDate}</h6>
					)}
							...
							<div>
								<ButtonGroup variant="contained">
									<Button
										startIcon=<EditIcon />
										onClick={() =>
											navigate(`/comment/edit/${comment.commentID}`)
										}
									>
										Edit
									</Button>
